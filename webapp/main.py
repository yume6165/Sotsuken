...
def anken_read_img(folder):#フォルダを指定して
	...
	for file in files:
		...
		#print(data_list)
	with open('D:\\Sotsuken\\webapp\\public\\result\\output_file\\img_infos.csv', 'w') as f:
		writer = csv.DictWriter(f, ['original_img', 'edge_img', 'color_hist_img', 'color'])
		#ヘッダの書き込み
		#writer.writeheader()
...
def mmm_operation(path, anken_path):
	...
	with open('D:\\Sotsuken\\webapp\\public\\result\\output_file\\context_dist\\0_no_context.csv', 'w') as f:
		writer = csv.writer(f)
